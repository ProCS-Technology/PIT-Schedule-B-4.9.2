...
{
    public partial class Send_Welcome_Email : System.Web.UI.Page
    {
        ...
        protected void GetUserList()
        {
            ...
            if (!String.IsNullOrEmpty(Convert.ToString(Session["CompanyId"])))
            {
                ...
                var _query = "SELECT A.USER_EMAIL,A.USER_NM,B.USER_TYPE,A.LOGIN_ID FROM  " + AdminDB + "..PROCS_USERS A INNER JOIN PROCS_INSIDER_USER B  ON A.LOGIN_ID=B.USER_LOGIN  WHERE ";
                            ...
                            foreach (DataRow row in dt.Rows)
                            {
                                ...
                                else if (row["USER_TYPE"].ToString() == "Application")
                                {
                                    ...
                                    EmailSender.SendMail(
                                        row["USER_EMAIL"].ToString(),
                                        TextBoxSubject.Text,
                                        TextareaAppTemplate.Value.Replace("[UserId]", row["LOGIN_ID"].ToString()).Replace("[USER_NM]", row["USER_NM"].ToString()),
                                        attachments, "Welcome Email", Convert.ToString(Session["CompanyId"]),
                                        strCC, null, Convert.ToString(Session["EmployeeId"])
                                    );
                                }
        ...
        protected void LinkButtonSaveTemplate_Click(object sender, EventArgs e)
        {
            if (!String.IsNullOrEmpty(Convert.ToString(Session["CompanyId"])))
            {
                ...
                using (SqlConnection con = new SqlConnection(ConnectionString))
                using (SqlCommand cmd = new SqlCommand("UPDATE PROCS_INSIDER_EMAILS_TEMPLATE_HDR SET TEMPLATE_BODY='" + TextareaAppTemplate.InnerHtml + "', TEMPLATE_SUBJECT='" + TextBoxSubject.Text + "' WHERE TEMPLATE_ID= " + ApplicationId + " AND COMPANY_ID=" + CompanyId, con))
                {
                    cmd.CommandType = System.Data.CommandType.Text;
