...
function GetAllUPSIGroup() {
    var memberType = $("#bindMemberType").val();
    
    $("#Loader").show();
   
    var webUrl = uri + "/api/UPSIGroupReport/GetUPSIMember";
    $.ajax({
        url: webUrl,
        type: "POST",
        async: false,
        data: JSON.stringify({
            GROUP_TYPE: memberType
            ...
            if (msg.StatusFl == true) {
                ...
                if (msg.StatusFl == true) {
                    ...
                    for (var i = 0; i < msg.UPSIMembersGroupList[0].listDesignatedMember.length; i++) {
                        result += '<option value="' + msg.UPSIMembersGroupList[0].listDesignatedMember[i].PAN + '">' + msg.UPSIMembersGroupList[0].listDesignatedMember[i].NAME + '(' + msg.UPSIMembersGroupList[0].listDesignatedMember[i].EMAIL+')</option>';

                    }


                    $("#ddlupsiDesignatedM").html('');
                    $("#ddlupsiDesignatedM").html(result);
...
function fnGetUPSIReport() {
    ...
    var members = $("#ddlupsiDesignatedM").val();
    if (members.length > 0) {
    ...
    var webUrl = uri + "/api/UPSIGroupReport/GetUPSIReportByMember";
    $.ajax({
        url: webUrl,
        type: "POST",
        contentType: "application/json; charset=utf-8",
        ...
        async: false,
        data: JSON.stringify({
            VALID_FROM: from_date, VALID_TLL: till_date, listDesignatedMember: membersPan
        }),
        success: function (msg) {
