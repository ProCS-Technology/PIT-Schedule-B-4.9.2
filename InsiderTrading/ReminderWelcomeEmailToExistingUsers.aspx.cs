...
{
    public partial class ReminderWelcomeEmailToExistingUsers : System.Web.UI.Page
    {
            ...
            using (SqlConnection con = new SqlConnection(getConnectionString))
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    using (SqlConnection con1 = new SqlConnection(getConnectionString))
                    {
                        ...
                        cmd1.Parameters.AddWithValue("@Mode", "GET_ALL_INSIDERS_NOT_COMPLETED_DECLARATION");
                        ...
                        if (ds1.Tables.Count > 0)
                        {
                            if (ds1.Tables[0].Rows.Count > 0)
                            {
                                foreach (DataRow dr in ds1.Tables[0].Rows)
                                {
                                    ...
                                    DataSet ds2 = SQLHelper.ExecuteDataset(SQLHelper.GetConnString(), CommandType.StoredProcedure, "SP_PROCS_IT_REMINDER_WELCOME_EMAIL_TEMPLATE", Convert.ToString(HttpContext.Current.Session["ModuleDatabase"]), parameters);
                                    ...
                                    //    defaultEmail, disclosureEmail, userEmail, subject, body, smtpHostName, ssl, smtpUserName, password, 
                                    //    userDefaultCredential, port
                                    //);
                                }
                            }
                        }
